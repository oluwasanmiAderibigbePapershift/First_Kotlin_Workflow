...
class ValidateFormLayoutRunner(private val binding: ActivityValidateFormBinding) : LayoutRunner<ValidateFormScreen> {
    override fun showRendering(rendering: ValidateFormScreen, viewEnvironment: ViewEnvironment) {
        ...
        binding.tlFirstName.error = if (rendering.firstNameError != -1) binding.root.context.getString(rendering.firstNameError) else ""
        ...
        binding.tlLastName.error = if (rendering.lastNameError != -1) binding.root.context.getString(rendering.lastNameError) else ""
        ...
        binding.tlAge.error = if (rendering.ageError!= -1) binding.root.context.getString(rendering.ageError) else ""
        ...
        binding.tlEmail.error = if (rendering.emailError != -1) binding.root.context.getString(rendering.emailError) else ""
